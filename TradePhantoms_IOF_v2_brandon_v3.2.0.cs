...
namespace TradePhantomsIOF
{
    ...
    public class TradePhantoms_IOF_v2 : Indicator, IVolumeAnalysisIndicator
    {
        ...
        private const string PendingEventsLogPath =
            @"D:\TradingHQ\data\logs\pending_events.jsonl";
        ...
        // 2026-05-15 (Phase 1.2.1): same dedupe pattern for zone_armed.
        // Previously each TryPublishIntent would emit zone_armed even on
        // repeated arms of the same zone (e.g., replay storms). The hub
        // expects at most one zone_armed per zone_id per indicator session.
        // Removed on invalidate so a re-detected zone can re-arm cleanly.
        private readonly HashSet<string> _emittedArmedZoneIds = new HashSet<string>(StringComparer.Ordinal);

        // 2026-05-15 (Phase 1.2.2): single-emit guard for observer_goodbye.
        // Dispose() calls OnClear() internally, so without this guard a
        // single chart close would fire TWO goodbye events. Reset on OnInit
        // so the next chart attach can fire a fresh goodbye on close.
        private bool _observerGoodbyeEmitted = false;
        ...
        private string _indicatorVersion = "v3.2.0-d-drive-paths";
        ...
        protected override void OnInit()
        {
            ...
            // this, the chart can't go LIVE until the next bar closes —            // which on a Saturday means waiting until Sunday evening.
            ...
            if (!_observerHelloEmitted)
            {
                ...
                _emittedArmedZoneIds.Clear();
                _observerGoodbyeEmitted = false;
                try
                {
        ...
        {
            // 2026-05-15 (Phase 1.2.2): emit observer_goodbye BEFORE state
            // teardown so the recorder can distinguish clean disconnects from
            // SILENT_DARK drops. Single-emit guard prevents double-fire when
            // Dispose() calls OnClear() — see _observerGoodbyeEmitted decl.
            if (!_observerGoodbyeEmitted)
            {
                _observerGoodbyeEmitted = true;
                try
                {
                    EmitObserverEvent("observer_goodbye", new
                    {
                        reason = "OnClear",
                    });
                }
                catch { /* defensive — never let goodbye crash the cleanup path */ }
            }
        ...
        private void TryPublishIntent(IofZone z, bool isDemand)
        {
            ...
            // 2026-05-15 (Phase 1.2.1): dedupe zone_armed emission per zone_id
            // per indicator session. Hub expects at most one zone_armed per
            // zone_id; repeated arms (replay storms, settings reloads) used to
            // multi-fire and inflate the recorder.
            if (string.IsNullOrEmpty(z.Id) || !_emittedArmedZoneIds.Add(z.Id)) return;
            ...
            EmitObserverEvent("zone_armed", new
            {
                ...
                // 2026-05-14 v3: geometry + provenance fields so hub/bot can
                // reconstruct the zone rect without re-deriving from bars.
                // Mirrors the bridge "intent" payload.
                zone_proximal            = isDemand ? z.BodyHi : z.BodyLo,
                zone_distal              = isDemand ? z.WickLo : z.WickHi,
                wick_start               = z.WickLo,
                wick_end                 = z.WickHi,
                // 2026-05-15: raw body edges so Python compute_entry can
                // use the FULL rectangle (proximal/distal alone is missing
                // one body edge — the "far" side — needed by Tp25/50/75
                // entry depth modes).
                zone_body_hi             = z.BodyHi,
                zone_body_lo             = z.BodyLo,
                zone_wick_hi             = z.WickHi,
                zone_wick_lo             = z.WickLo,
                zone_type                = z.Type.ToString(),
                zone_created_at          = z.EndTime.AddSeconds(barSeconds)
                                            .ToUniversalTime()
                                            .ToString("o", System.Globalization.CultureInfo.InvariantCulture),
                source                   = "indicator_5m",
            });
        ...
        // ---------------------------------------------------------------------
        // 2026-05-15 (Phase 1.2.4): surface swallowed exceptions to a dedicated
        // forensic log so silent failures stop being invisible. Most network-
        // emit best-effort catches stay as bare catch { } (justified — too
        // noisy if logged). Reserve LogSwallowed for sites where a failure
        // genuinely hides a bug (heartbeat fallback, launcher failure).
        private static readonly object _swallowedLogLock = new object();
        private static void LogSwallowed(string site, Exception ex)
        {
            try
            {
                string path = @"D:\TradingHQ\data\logs\indicator_swallowed.log";
                ...
                lock (_swallowedLogLock)
                {
                    System.IO.File.AppendAllText(path, line);
                }
            }
            catch { /* if even this fails we can't help further */ }
        }
        ...
        private void EmitHeartbeat()
        {
            try
            {
                ...
                // 2026-05-15 (Phase 1.2.3): replaced network heartbeat POST
                // with a local liveness-log file append. Recorder polls the
                // file mtime to detect SILENT_DARK without network dependency.
                // Format: <ISO>\t<observer_id>\t<symbol>\t<last_bar_ts>\t<active_zones>
                try
                {
                    string symbolRoot = "";
                    try { symbolRoot = this.Symbol?.Root ?? this.Symbol?.Name ?? ""; } catch { }
                    string line = string.Format(
                        System.Globalization.CultureInfo.InvariantCulture,
                        "{0}\t{1}\t{2}\t{3}\t{4}\n",
                        ...
                        lastBarIso,
                        activeZoneCount);
                    string livenessPath = @"D:\TradingHQ\data\logs\indicator_liveness.log";
                    System.IO.Directory.CreateDirectory(System.IO.Path.GetDirectoryName(livenessPath));
                    System.IO.File.AppendAllText(livenessPath, line);
                }
                catch (Exception lhEx) { LogSwallowed("EmitHeartbeat.livenessLog", lhEx); }
            }
            catch { /* never throw */ }
        }
            ...
            try
            {
                ...
                try
                {
                    ...
                    foreach (var kv in _previousZoneSnapshots)
                    {
                        ...
                        // 2026-05-15 (Phase 1.2.1): also clear from armed-dedupe
                        // set so a re-detected zone can re-arm cleanly.
                        _emittedZoneDetectedIds.Remove(kv.Key);
                        _emittedArmedZoneIds.Remove(kv.Key);
        ...
        // window. Logs land in D:\TradingHQ\data\logs\backend.log.
        // ---------------------------------------------------------------------
        private static void EnsureBridgeBackendRunning()
        {
            ...
            catch (Exception vbsEx) { LogSwallowed("BridgeLauncherVbs.Start", vbsEx); }
        }
        ...
        private void ScanZones()
        {
            ...
            for (int i = 0; i < this.chartZones.Count; i++)
            {
                ...
                {
                    // 2026-05-15 (Phase 1.2.7): cold-start backfill throttle.
                    // OnInit historical replay can emit 200-500 zone_detected
                    // events in 1-2 seconds, overwhelming the recorder. Gate
                    // by isReplayingTrendHistory so live emissions are not
                    // affected. ~10ms per emit slows the burst from 2s to 4s.
                    if (this.isReplayingTrendHistory)
                    {
                    ...
                    EmitObserverEvent("zone_detected", new
                    {
                        ...
                        // 2026-05-15 (Phase 1.2.7): forensic flag so the
                        // recorder can distinguish cold-start bursts from
                        // live emissions when investigating issues.
                        during_backfill          = this.isReplayingTrendHistory,
                    });
                }
            }
                               ...
                               Brush labelBg, Brush labelFg)
        {
            ...
            for (int zi = this.zones.Count - 1; zi >= 0; zi--)
            {
                ...
                if (this.ShowZoneScoreLabels)
                {
                    ...
                    // 2026-05-13: pre-arm tag — clearest text confirmation                    // that this is the zone PASS C will arm next when price
        ...
        private TPUI.DashboardSettings BuildDashboardSettings()
        {
            ...
            {
                IndicatorVersion = $"TradePhantoms IOF {_indicatorVersion}",
                Ticker           = Symbol?.Name ?? "",
