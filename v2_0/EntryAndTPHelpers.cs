...
namespace TradePhantomsIOF
{
    ...
    public static class EntryTPMath
    {
            ...
            double slippagePoints = 0.0)
        {
            ...
            // v2.0: real-world slippage buffer. Stop price is unchanged — we
            // trade fewer contracts so (stop loss + slippage) stays within budget.
            double effectiveSlDist = slDist + Math.Max(0.0, slippagePoints);

            // Dollar risk per single contract = effectiveSlDist * pointValue.
            // Number of whole contracts the budget supports = dollarRisk / that.
            double riskPerContract = effectiveSlDist * pointValue;
