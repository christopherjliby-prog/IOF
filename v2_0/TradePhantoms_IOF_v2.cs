// =============================================================================
// TradePhantoms_IOF_v2.cs — IOF v2.0 (Real-World Execution Edition)
// =============================================================================
// Version : 2.0.0
// Date    : 2026-05-18
// Author  : Christopher (Nfifty4) + Christopher's Claude (Sonnet 4.6)
//
// WHAT'S NEW IN v2.0 vs v2.1.3-tp-entries
// ─────────────────────────────────────────
// 1. SLIPPAGE-ADJUSTED POSITION SIZING
//    New input: "Slippage buffer (points)" — default 10.
//    Contract size is calculated as:
//      floor(DollarRisk / ((stop_distance + SlippagePoints) × point_value))
//    Stop price is UNCHANGED. Fewer contracts are traded so total exposure
//    including worst-case slippage stays within DollarRiskPerTrade.
//    Set SlippagePoints = 0 to revert to v1 behavior.
//
// 2. COUNTER-TREND TRADES: 1:1 TARGET ONLY
//    When a zone is counter-trend (demand in bear / supply in bull), the
//    TP structure is automatically capped at 1 TP (1:1 R:R). No TP2/TP3.
//    With-trend trades run the full cascade as configured.
//
// 3. MINIMUM STOP BUFFER ENFORCED: 2 TICKS HARD FLOOR
//    If StopBufferTicks is set below 2 in the UI, it is silently raised
//    to 2 on init. Prevents stops from sitting inside the zone's natural
//    price range and absorbs 1-tick real-world stop fill slippage.
//
...
namespace TradePhantomsIOF
{
    ...
    public class TradePhantoms_IOF_v2 : Indicator, IVolumeAnalysisIndicator
    {
        ...
        // v2.0: Real-world slippage buffer in points. Stop price is unchanged —
        // contract size is reduced so (stop loss + slippage) stays within
        // DollarRiskPerTrade. Set to 0 to disable (reverts to v1 behavior).
        ...
        {
            ShortName = $"IOFv2.0 (>={MinScore}/21)";

            // v2.0: Hard floor — stop buffer must be at least 2 ticks to absorb
            // real-world fill slippage and stay outside the zone after execution.
            if (this.StopBufferTicks < 2) this.StopBufferTicks = 2;
        ...
        private void TryPublishIntent(IofZone z, bool isDemand)
        {
            ...
            // v2.0: counter-trend trades cap at 1 TP (1:1 R:R only).
            bool isCounterTrend = (this.trendStateMachine != null) && (
            ...
            int effectiveTpCount = isCounterTrend ? 1 : this.TpCount;

            var tps = TradePhantomsIOF.EntryTPMath.ComputeTPs(
                isDemand, entry, zoneHeight, effectiveTpCount, this.TpStep);
